...
void VS()
{
    ...
    vNormal = (vec4(vNormal, 0.0) * cView).xyz;
    vWorldPos = vec4(worldPos, 1.0) * cView;
#elif defined(PREOUTLINE)
    vNormal = GetWorldNormal(modelMatrix);
    vWorldPos = vec4(worldPos, gl_Position.z / gl_Position.w);
#else
    vNormal = GetWorldNormal(modelMatrix);
	vWorldPos = vec4(worldPos, GetDepth(gl_Position));
#endif   
    vModelViewPos = cView * vWorldPos;

#if defined(NORMALMAP) || defined(DIRBILLBOARD)
	vec4 tangent = GetWorldTangent(modelMatrix);
	vec3 bitangent = cross(tangent.xyz, vNormal) * tangent.w;
	vNorCoord = vec4(GetNorCoord(iTexCoord), bitangent.xy);
	//vNorCoord = vec4(GetTexCoord(iTexCoord), bitangent.xy);
	vTangent = vec4(tangent.xyz, bitangent.z);
#endif

	vTexCoord = GetTexCoord(iTexCoord);    
    vOriginTexCoord = vec2(dot(iTexCoord, vec2(1,0)) , dot(iTexCoord, vec2(0,1)) );

#if defined INSTANCED_COLOR
    cMatDiffColor = iMatDiffColor;
#endif
...
#define COMPILEPS
    ...
    #define NUM_ENV_SAMPLES 6
    // Additional samples added for rough reflection & refraction
    #define NUM_ENV_SAMPLES_ROUGH 6

    // Conductor Fresnel values for sheen
    #define SHEEN_N 2.9114
    #define SHEEN_K 3.0893

    uniform int cMaterialType=0;
    uniform sampler2D sRadianceMap;
    uniform sampler2D sIrradianceMap;
    uniform int cEnvRadianceMips = 10;
	uniform sampler2D sCUSTOM5;
	uniform sampler2D sCUSTOM6;

    uniform float cBase_weight = 1.000000;
    uniform float cDiffuse_roughness =0.000000;

    uniform float cSpecular_weight = 1.000000;
    uniform vec3  cSpecular_color = vec3(1.000000, 1.000000, 1.000000);
    uniform float cSpecular_IOR = 1.600000;
    uniform float cSpecular_anisotropy = 0.000000;
    uniform float cSpecular_rotation = 0.000000;
    uniform float cSheen_weight = 0.000000;
    uniform vec3  cSheen_color = vec3(1.000000, 1.000000, 1.000000);
    uniform float cSheen_roughness = 0.300000;
    uniform float cCoat_weight = 0.000000;
    uniform vec3  cCoat_color = vec3(1.000000, 1.000000, 1.000000);
    uniform float cCoat_roughness = 0.100000;
    uniform float cCoat_IOR = 1.500000;
    uniform float cRefract_weight = 1.000000;
    uniform vec3  cRefract_color = vec3(0.0, 0.0, 0.0);
    uniform float cRefract_roughness = 1.00000;
    uniform int cOutlineEnable = 0;
    ...
    vec3 fixNormal( vec3 worldPos, vec2 texCoord, vec3 N0, sampler2D map, float intensity, bool bumpEnabled, mat3 TBN1,sampler2D spMap, vec2 spTexCoord,bool seamPuckeringNormalEnabled  ) 
    {
        const float TEX_DELTA = 0.0001;

       // mat3 TBN1 = cotangent_frame(N0, worldPos, texCoord);
       // mat3 TBN2 = cotangent_frame(N0, worldPos, spTexCoord);

        vec3 normalOffset = N0;
        vec3 seamPuckeringVec = vec3(0.0, 0.0, 1.0);

        if( bumpEnabled) 
        {
            normalOffset = texture( map, texCoord ).xyz;
            normalOffset = normalize(normalOffset * 2. - 1.);
            ...
            {
                //normalOffset.x = normalOffset.x * sign(intensity);
                //normalOffset.y = normalOffset.y * sign(intensity);
                //normalOffset.z = 1.0 * normalOffset.z / ((abs(intensity) - 1.0)*0.3 + 1.0);
                normalOffset.xy = normalOffset.xy*intensity;
                 normalOffset = normalize(normalOffset);
            }
            normalOffset = normalize(TBN1*normalOffset);
        }
        if (seamPuckeringNormalEnabled) 
        {
            seamPuckeringVec = texture( spMap, spTexCoord ).xyw;
            seamPuckeringVec = seamPuckeringVec * 2.0 - 1.0;
            seamPuckeringVec.z = seamPuckeringVec.z * 0.8;

            vec3 posinEye = vModelViewPos.xyz;
            mat3 TBN2 = cotangent_frame(N0, posinEye, spTexCoord);
            seamPuckeringVec = normalize(TBN2*seamPuckeringVec);
        }

        if(bumpEnabled && seamPuckeringNormalEnabled) 
        {
            float z0 = dot(N0, seamPuckeringVec);
            float z1 = dot(N0, normalOffset);
            return normalize(seamPuckeringVec - z0 * N0 + normalOffset - z1 * N0 + z0 * z1 * N0);
        }
        else if ( bumpEnabled  )
        {
            return normalOffset;
        }
        else if (seamPuckeringNormalEnabled)
        {
    ...
    float engGetMaxEnvLOD(sampler2D map) 
    { 
        ivec2 envSize = textureSize(sZoneCubeMap, 0);
    ...
    vec2 eng_latlong_projection(vec3 dir)
    {
    ...
    vec3 eng_latlong_map_lookup(vec3 dir, mat4 transform, float lod, sampler2D envSampler)
    {
        vec3 envDir = normalize((transform * vec4(dir,0.0)).xyz);
        vec2 uv = eng_latlong_projection(envDir);
    ...
    vec3 engTextureEnvMapLOD(vec3 dir, float lod) 
    {
        ...
        //    color = textureLod(sZoneCubeMap, texcoord, lod).xyz;
        //#else
        //    color = textureLod(sZoneCubeMap, dir, lod).xyz;
        //#endif
       // textureLod( map, getTexCoord(angle0), lod );
        //return rgb_from_srgb(color);      
        float radian = cIblAngle / 180.0 * PI; 
        float cosTheta = cos(radian); 
        float sinTheta = sin(radian) ;        
        mat4 u_envMatrix = mat4(cosTheta, 0.000000, sinTheta, 0.000000, 
                            0.000000, 1.000000, 0.000000, 0.000000, 
                            -sinTheta, 0.000000, cosTheta, 0.000000, 
                             0.000000, 0.000000, 0.000000, 1.000000);
        return eng_latlong_map_lookup(dir, u_envMatrix, lod, sRadianceMap)*cIblIntensity;    
    }

    // Diffuse environment light (averaged over the hemisphere)
    vec3 engEnvIrradiance(vec3 dir) 
    {
        //float maxLod = max(engGetMaxEnvLOD(sIrradianceMap)-1,0);
        //return textureLod(sIrradianceMap, vec2(0.5,0.5), maxLod).rgb;
       // return eng_latlong_map_lookup(vec3(0,0,1), u_envMatrix, 0.0, sIrradianceMap)*cIblIntensity;

        //float lod = max(0.0, engGetMaxEnvLOD() - 0.0);
        //return rgb_from_srgb(textureLod(sZoneCubeMap, dir, lod).xyz); 
        float radian = cIblAngle / 180.0 * PI; 
        float cosTheta = cos(radian); 
        float sinTheta = sin(radian) ;        
        mat4 u_envMatrix = mat4(cosTheta, 0.000000, sinTheta, 0.000000, 
                            ...
                            -sinTheta, 0.000000, cosTheta, 0.000000, 
                             0.000000, 0.000000, 0.000000, 1.000000);
        return eng_latlong_map_lookup(dir, u_envMatrix, 0.0, sIrradianceMap)*cIblIntensity;      
    }
    ...
    {       
        ...
        if (numSamples < 2) 
            ...
            return max(0.0, (cEnvRadianceMips - 0.5 * log2(1.0 + float(numSamples) * p * INV_2PI * distortion)));
    ...
    vec2 rand(VRayMtlContext ctx, int sampleIdx, int nbSamples) 
    {
        ...
        return vec2(fract(float(sampleIdx + 1) * invPlast),	float(sampleIdx) / float(nbSamples) + ctx.fragmentNoise);
    }
...
/// https://seblagarde.wordpress.com/2013/04/29/memo-on-fresnel-equations/
    ...
    vec3 sampleBRDF(VRayMtlInitParams params, VRayMtlContext ctx, int sampleIdx, int nbSamples, out float rayProb, out float brdfContrib) {
    ...
    vec3 sampleRefractBRDF(VRayMtlInitParams params, VRayMtlContext ctx, int sampleIdx, int nbSamples, out bool totalInternalReflection) {
        vec3 geomNormal = ctx.geomNormal;
    ...
    VRayMtlContext initVRayMtlContext(VRayMtlInitParams initParams) {
        ...
        //result.diff = mix(result.diff, vec3(0.04), metalness);  
        ...
        if (result.hasSheen) {
            float albedoView = sheenDirectionalAlbedo(max(0.3, dot(-result.e, geomNormal)), sheenGloss);
...
void PS()
{
    ...
    mat3 tbn;
        ...
        vec3 bitangent = vec3(vNorCoord.zw, vTangent.w);
        tbn = mat3(tangent, bitangent, vNormal);

        if(cUseParallaxMap==1)
        {
            tbn = transpose(mat3(vTangent.xyz, bitangent, vNormal));
            vec3 viewDirWS = cCameraPosPS.xyz - vWorldPos.xyz;
            //PARALLAX_OCCLUSION
            float VdotN = dot(normalize(viewDirWS), vNormal);           
            vec3 viewDir = tbn * viewDirWS;
            //vec2 plxCoords = ParallaxOffsetLimit(sEnvMap, vNorCoord.xy, viewDir);
            vec2 plxCoords = ParallaxOcclusionMapping(sEnvMap, texCoord, viewDir, VdotN);
    ...
    //计算缝线法线    
    N0 = normalize(vNormal);
    N = fixNormal(vWorldPos.xyz, vNorCoord.xy, N0, sNormalMap, cNormalScale, cUseNormMap==1?true:false, tbn,sCUSTOM6, vOriginTexCoord,cUseSeamNorm);    //cUseSeamNorm   
    
    // shader parameter 赋值
    #ifdef DIFFMAP //albedo 贴图	  
		if (cUseDiffMap == 1)
		{
			vec4 diffInput = texture2D(sDiffMap, texCoord);
			diffuse = cMatDiffColor * diffInput;                    
		}
		else
		{
			diffuse = cMatDiffColor;           
		}
    ...
    if(cUseMetalMap==1)    
        metalness = cMetallic*texture2D(sSpecMap, texCoord).r; 
    
    if(cUseRoughnessMap==1)    
        roughness = cRoughness*texture2D(sEmissiveMap, texCoord).r;  
	
	float alpha = diffuse.a;
	if (cUseOpacityMap == 1)
		alpha = texture2D(sCUSTOM5, texCoord).r;
    alpha = sqrt(alpha);
	//变换到线性空间  
   // roughness *=roughness;  
    diffuse.rgb = ToGamma(diffuse.rgb); 
    V = normalize(cCameraPosPS - vWorldPos.xyz);
   

    // Init VRayMtl with defaults
	VRayMtlInitParams initParams;
	initParams.Vw = V;
	initParams.geomNormal = N;

	initParams.diffuseColor = diffuse.rgb;
	initParams.diffuseAmount = cBase_weight;
	initParams.roughness = cDiffuse_roughness;
	initParams.selfIllum = vec3(0);
	initParams.reflColor = cSpecular_color;
	initParams.reflAmount = cSpecular_weight;
	initParams.reflGloss = min(roughness,0.9);
	initParams.traceReflections = true;
	initParams.metalness =metalness;
	initParams.aniso = cSpecular_anisotropy;
	initParams.anisoRotation = cSpecular_rotation;
	initParams.anisoAxis = 2;
	initParams.opacity = vec3(1, 1, 1);
	initParams.refractionColor = cRefract_color;
	initParams.refractionAmount = cRefract_weight;
	initParams.refrGloss = 1-cRefract_roughness ;
	initParams.refractionIOR = cSpecular_IOR;
	initParams.traceRefractions = true;
	initParams.useFresnel = true;
	initParams.fresnelIOR = cSpecular_IOR;
	initParams.lockFresnelIOR = true;
	initParams.doubleSided = true;
	initParams.useRoughness = true;
	initParams.gtrGamma = 2.2;
	initParams.brdfType = 3;
	initParams.fogColor = vec3(0.0, 0.0, 0.0);
	initParams.fogBias = 0.0;
	
	initParams.sheenAmount = cSheen_weight;
    initParams.sheenColor =cSheen_color; 
    initParams.sheenGlossiness =cSheen_roughness;
    initParams.coatColor=cCoat_color;
    initParams.coatAmount =cCoat_weight;
    initParams.coatGlossiness=cCoat_roughness;       
	initParams.coatIOR = cCoat_IOR;

	initParams.thinFilmThickness = 0;
    ...
    //着色计算
        ...
        #if defined(DIRLIGHT)
            atten = GetAtten(N, vWorldPos.xyz, lightDir);
        #elif defined(SPOTLIGHT)
            atten = GetAttenSpot(N, vWorldPos.xyz, lightDir);
        #else
            atten = GetAttenPoint(N, vWorldPos.xyz, lightDir);
        #endif
        ...
        #else
            //lightColor = pow(cLightColor.rgb, vec3(GAMMA)); 
            lightColor = cLightColor.rgb;
        #endif
       

	    vec3 incident = shadow * lightColor * atten;   

       	//float alpha = diffuse.a;
        vec3 diffuseDirect = computeDirectDiffuseContribution(initParams, ctx, lightDir)*incident;      
        vec3 reflDirect = computeDirectReflectionContribution(initParams, ctx, lightDir)*incident;
        #ifdef ALPHA                    
            reflDirect *= alpha;
        #endif
        ...
        vec3 sheen = vec3(0.0);
        if (ctx.hasSheen) {
            sheen = computeDirectSheenContribution(initParams, ctx, lightDir)*incident;           
        }

        vec3 coat = vec3(0.0);
        if (ctx.hasCoat) {
            coat = computeDirectCoatContribution(initParams, ctx, lightDir)*incident;           
        }
       
        vec3 refraction = computeRefractFogContrib(initParams, ctx, diffuseDirect)*incident; 
        vec3 color = diffuseDirect * ctx.diff + reflDirect * ctx.refl + ctx.illum + refraction * ctx.refr + sheen * ctx.sheen + coat * ctx.coat;
       
        gl_FragColor = vec4(color, alpha); 
    #elif defined(PREPASS)
		vec3 vn = normalize(vNormal);
        gl_FragData[0] = vec4(vn * 0.5 + 0.5, 1.0);
        gl_FragData[1] = vec4(EncodeLinearDepth(vWorldPos.z), 0.0);
    #elif defined(DEFERRED)
        // Fill deferred G-buffer
		vec3 specColor = vec3(0.04);
		specColor = mix(specColor, baseColor.rgb, metallic);
        ...
        gl_FragData[2] = vec4(N * roughness, spareData.b);
        gl_FragData[3] = vec4(EncodeDepth(vWorldPos.w), 0.0);
    #elif defined(PREOUTLINE)
        if (cOutlineEnable == 0)
            discard;
        else {
            gl_FragColor = vec4(vWorldPos.w);
        }
    #else 

        //float alpha = diffuse.a;       
        vec3 diffuseIndirect = computeIndirectDiffuseContribution(initParams, ctx)*1;
        vec3 reflIndirect = computeIndirectReflectionContribution(initParams, ctx)*1.0;
        #ifdef ALPHA                    
            reflIndirect *= alpha;
       #endif
        vec3 sheen = vec3(0.0);
        if (ctx.hasSheen) {          
            sheen = computeIndirectSheenContribution(initParams, ctx);            
        }

        vec3 coat = vec3(0.0);
        if (ctx.hasCoat) {           
            coat = computeIndirectCoatContribution(initParams, ctx);           
        }

  
        vec3 refraction = computeIndirectRefractionContribution(initParams, ctx, alpha, -initParams.Vw);

        vec3 color = diffuseIndirect * ctx.diff + reflIndirect * ctx.refl + ctx.illum + refraction * ctx.refr + sheen * ctx.sheen + coat * ctx.coat;
		gl_FragColor = vec4(color, alpha);
