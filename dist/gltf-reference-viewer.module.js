...
var shaderFunctions = "#define GLSLIFY 1\nconst float M_PI=3.141592653589793;in vec3 v_Position;\n#ifdef HAS_NORMALS\n#ifdef HAS_TANGENTS\nin mat3 v_TBN;\n#else\nin vec3 v_Normal;\n#endif\n#endif\n#ifdef HAS_VERTEX_COLOR_VEC3\nin vec3 v_Color;\n#endif\n#ifdef HAS_VERTEX_COLOR_VEC4\nin vec4 v_Color;\n#endif\nstruct AngularInfo{float NdotL;float NdotV;float NdotH;float LdotH;float VdotH;vec3 padding;};vec4 getVertexColor(){vec4 color=vec4(1.0,1.0,1.0,1.0);\n#ifdef HAS_VERTEX_COLOR_VEC3\ncolor.rgb=v_Color;\n#endif\n#ifdef HAS_VERTEX_COLOR_VEC4\ncolor=v_Color;\n#endif\nreturn color;}struct NormalInfo{vec3 n;vec3 t;vec3 b;vec3 ng;vec3 tg;vec3 bg;};NormalInfo getNormalInfo(vec3 v){vec2 UV=getNormalUV();\n#ifdef HAS_TANGENTS\nmat3 tbn=v_TBN;\n#else\nvec3 pos_dx=dFdx(v_Position);vec3 pos_dy=dFdy(v_Position);\n#ifdef HAS_NORMALS\nvec3 ng=normalize(v_Normal);\n#else\nvec3 ng=cross(pos_dx,pos_dy);\n#endif\nvec3 tex_dx=dFdx(vec3(UV,0.0));vec3 tex_dy=dFdy(vec3(UV,0.0));vec3 t=(tex_dy.t*pos_dx-tex_dx.t*pos_dy)/(tex_dx.s*tex_dy.t-tex_dy.s*tex_dx.t);t=normalize(t-ng*dot(ng,t));vec3 b=normalize(cross(ng,t));mat3 tbn=mat3(t,b,ng);\n#endif\nif(dot(v,tbn[2])<0.0){tbn*=-1.0;}NormalInfo info;info.tg=normalize(tbn[0].xyz);info.bg=normalize(tbn[1].xyz);info.ng=normalize(tbn[2].xyz);\n#ifdef HAS_NORMAL_MAP\nvec3 n=texture(u_NormalSampler,UV).rgb;n=2.0*n-1.0;n*=vec3(u_NormalScale,u_NormalScale,1.0);info.n=normalize(tbn*n);info.t=info.tg;info.b=info.bg;\n#else\ninfo.t=info.tg;info.b=info.bg;info.n=info.ng;\n#endif\nreturn info;}AngularInfo getAngularInfo(vec3 pointToLight,vec3 normal,vec3 view){vec3 n=normalize(normal);vec3 v=normalize(view);vec3 l=normalize(pointToLight);vec3 h=normalize(l+v);float NdotL=clamp(dot(n,l),0.0,1.0);float NdotV=clamp(dot(n,v),0.0,1.0);float NdotH=clamp(dot(n,h),0.0,1.0);float LdotH=clamp(dot(l,h),0.0,1.0);float VdotH=clamp(dot(v,h),0.0,1.0);return AngularInfo(NdotL,NdotV,NdotH,LdotH,VdotH,vec3(0,0,0));}float clampedDot(vec3 x,vec3 y){return clamp(dot(x,y),0.0,1.0);}float sq(float t){return t*t;}vec2 sq(vec2 t){return t*t;}vec3 sq(vec3 t){return t*t;}vec4 sq(vec4 t){return t*t;}vec3 transmissionAbsorption(vec3 v,vec3 n,float ior,float thickness,vec3 absorptionColor){vec3 r=refract(-v,n,1.0/ior);return exp(-absorptionColor*thickness*dot(-n,r));}"; // eslint-disable-line
